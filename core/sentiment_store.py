...
This store is intentionally small and TCC-safe:
- writes to /tmp/internmailer_db by default
- stores article-level items and topic-level snapshots
- provides compact history reads for the dashboard
"""
...
DEFAULT_SENTIMENT_DB = Path("/tmp/internmailer_db/market_sentiment.db")


def resolve_sentiment_db_path(db_path: Optional[str] = None) -> str:
    """Resolve a TCC-safe sentiment database path."""
    if db_path:
        path = Path(db_path)
        if not path.is_absolute():
            path = Path("/tmp/internmailer_db") / path
    else:
...
class SentimentStore(DatabaseManager):
    """Persistence layer for market sentiment data."""

    def __init__(self, db_path: Optional[str] = None):
        super().__init__(resolve_sentiment_db_path(db_path))
    ...
    def upsert_items(self, items: Iterable[SentimentItem]) -> int:
        """
        Insert or update the latest article-level sentiment items.

        Returns:
            Number of *new* items inserted (deduped by item_id).
        """
        ...
        with self.get_connection() as conn:
            # Compute "new vs existing" so callers can show a meaningful inserted count.
    ...
    def get_latest_snapshot(self, topic: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """Return the latest snapshot for a topic, including recent items."""
        if topic:
            row = self.fetch_one(
                """
                SELECT *
    ...
    def get_recent_items(
        ...
        """Return recent scored items, optionally filtered to one topic and/or time window."""
    ...
    def list_topics(self) -> List[str]:
        """Return topics ordered by latest update."""
        rows = self.fetch_all(
            """
    ...
    def get_history(self, limit: int = 10, topic: Optional[str] = None) -> List[Dict[str, Any]]:
        """Return a compact snapshot history for the dashboard."""
        if topic:
            rows = self.fetch_all(
                """
                SELECT *
    ...
    def log_run(
        ...
        """Record a refresh cycle in the run log."""
        with self.get_connection() as conn:
            conn.execute(
                """
