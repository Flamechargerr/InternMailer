#!/usr/bin/env python3
"""
Ultra Personalized Mass Mailing - InternMailing System
Send emails with research area inference + actual publication data for maximum personalization
"""
...
# Import enhanced research area inference and publication finder
...
def get_ultra_personalized_professors(count=10):
    """Get professors with ultra-personalization (research area + publications)"""
    try:
        df = pd.read_csv('data/scraped_professors_final.csv')
        
        # Initialize enhanced research area inference and publication finder
        ...
        # Target professors with known research areas (priority list)
        target_names = [
            # Machine Learning / AI
        ...
        if len(targeted_professors) < count:
            ...
            sample_professors = remaining_professors.sample(min(remaining_needed * 3, len(remaining_professors)))
        ...
        # Get publications for all professors
        print("🔍 Fetching recent publications for professors...")
        publications_data = publication_finder.get_publications_for_professors(targeted_professors)
        ...
        for professor_data in targeted_professors:
            ...
            # Get publications for this professor
            professor_name = professor_data.get('name', '')
            publications = publications_data.get(professor_name, [])
            
            # Create ultra-personalized professor data
            professor_info = {
        ...
        print(f"Error getting ultra personalized professors: {e}")
        return []

def get_hr_contacts_batch(count=3):
...
def create_ultra_personalized_academic_email(professor_data):
    """Create ultra-personalized academic professor template email with research area + publications"""
    template_content = load_html_template('templates/enhanced_academic_research_template.html')
    
    # Prepare publications for template
    publications_html = ""
    if professor_data.get('recent_publications'):
        publications_html = "<h3>🎯 Recent Research Publications</h3>"
        publications_html += "<div class='publications-section'>"
        for i, pub in enumerate(professor_data['recent_publications'][:3], 1):
            publications_html += f"""
            <div class='publication-item'>
                <h4>{i}. {pub['title']} ({pub['year']})</h4>
                {f"<p><strong>Venue:</strong> {pub['venue']}</p>" if pub.get('venue') else ""}
                {f"<p><strong>Summary:</strong> {pub['summary']}</p>" if pub.get('summary') else ""}
            </div>
            """
...
def log_sent_email(recipient_data, email_type, success):
    ...
    log_entry = {
        ...
        'publications_count': len(recipient_data.get('recent_publications', [])),
        'success': success
    }
...
def main():
    """Main function to send ultra-personalized emails"""
    print("🚀 ULTRA PERSONALIZED MASS MAILING - INTERNMAILING SYSTEM")
    print("=" * 60)
    print("Sending emails with RESEARCH AREA + ACTUAL PUBLICATIONS")
    print("=" * 60)
    
    # Get recipients
    professors = get_ultra_personalized_professors(10)
    hr_contacts = get_hr_contacts_batch(3)
    ...
    for area, profs in research_areas.items():
        ...
        for prof in profs[:2]:  # Show first 2 from each area
            pub_count = len(prof.get('recent_publications', []))
            print(f"     - {prof['first_name']} {prof['last_name']} from {prof['university']} ({pub_count} pubs)")
        if len(profs) > 2:
            print(f"     ... and {len(profs) - 2} more")
    ...
    results = {
        ...
        'hr_failed': 0,
        'research_area_stats': {},
        'publications_stats': {'with_pubs': 0, 'without_pubs': 0}
    }
    
    # Send professor emails
    if professors:
        print("🎓 SENDING ULTRA PERSONALIZED PROFESSOR EMAILS...")
        print("-" * 40)
        for i, professor in enumerate(professors, 1):
            ...
            pub_count = len(professor.get('recent_publications', []))
            if pub_count > 0:
                print(f"   📄 Recent Publications: {pub_count} papers found")
                results['publications_stats']['with_pubs'] += 1
            else:
                print(f"   ⚠️ No recent publications found")
                results['publications_stats']['without_pubs'] += 1
            
            subject, html_content = create_ultra_personalized_academic_email(professor)
            success = send_html_email_with_cv(
                professor['email'], 
                subject, 
                html_content, 
                "Ultra Personalized Academic Professor", 
    ...
    print()
    print("=" * 60)
    print("📊 ULTRA PERSONALIZED MAILING RESULTS:")
    print("=" * 60)
    ...
    total_profs = results['publications_stats']['with_pubs'] + results['publications_stats']['without_pubs']
    if total_profs > 0:
        pub_rate = (results['publications_stats']['with_pubs'] / total_profs * 100)
        print(f"   • Professors with recent publications: {results['publications_stats']['with_pubs']}/{total_profs} ({pub_rate:.1f}%)")
        print(f"   • Professors without publications: {results['publications_stats']['without_pubs']}/{total_profs}")
    
    print()
    
    if results['total_sent'] == total_recipients:
        print("🎉 ALL ULTRA PERSONALIZED EMAILS SENT SUCCESSFULLY!")
        print("✅ Ultra personalized mailing system is working perfectly!")
        print("🎯 All professor emails have research area + publication personalization!")
        print("📄 Publications integrated for maximum accuracy!")
        print("🚀 Ready to scale up to even larger campaigns!")
    elif results['total_sent'] >= total_recipients * 0.8:
        print("✅ Most ultra personalized emails sent successfully!")
        print("⚠️ Some emails failed - check errors above")
        print("🔧 Fix any issues before scaling up further")
    else:
        print("❌ Multiple ultra personalized emails failed")
        print("🔧 Please fix issues before proceeding")
    
    print("=" * 60)
    print("📋 NEXT STEPS:")
    print("1. Check email delivery and verify ultra personalization")
    print("2. Monitor for responses from publication-enhanced emails")
    print("3. If successful, expand to 50+ professor campaigns")
    print("4. Continue with even larger ultra-personalized campaigns")
    print("=" * 60)
